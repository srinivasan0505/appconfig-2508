...
func DeleteextensionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteextensionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_extensions_ExtensionIdentifier",
		mcp.WithDescription("Deletes an AppConfig extension. You must delete all associations to an extension before you delete the extension."),
		...
		mcp.WithString("ExtensionIdentifier", mcp.Required(), mcp.Description("The name, ID, or Amazon Resource Name (ARN) of the extension you want to delete.")),
		mcp.WithNumber("version", mcp.Description("A specific version of an extension to delete. If omitted, the highest version is deleted.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteextensionHandler(cfg),
	}
}
