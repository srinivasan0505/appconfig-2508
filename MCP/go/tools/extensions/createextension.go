...
func CreateextensionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateExtensionrequest
		...
		url := fmt.Sprintf("%s/extensions", cfg.BaseURL)
...
func CreateCreateextensionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_extensions",
		...
		mcp.WithNumber("Latest-Version-Number", mcp.Description("You can omit this field when you create an extension. When you create a new version, specify the most recent current version number. For example, you create version 3, enter 2 for this field.")),
		mcp.WithObject("Parameters", mcp.Description("Input parameter: The parameters accepted by the extension. You specify parameter values when you associate the extension to an AppConfig resource by using the <code>CreateExtensionAssociation</code> API action. For Lambda extension actions, these parameters are included in the Lambda request object.")),
		mcp.WithObject("Tags", mcp.Description("Input parameter: Adds one or more tags for the specified extension. Tags are metadata that help you categorize resources in different ways, for example, by purpose, owner, or environment. Each tag consists of a key and an optional value, both of which you define. ")),
		mcp.WithObject("Actions", mcp.Required(), mcp.Description("Input parameter: The actions defined in the extension.")),
		mcp.WithString("Description", mcp.Description("Input parameter: Information about the extension.")),
		mcp.WithString("Name", mcp.Required(), mcp.Description("Input parameter: A name for the extension. Each extension name in your account must be unique. Extension versions use the same name.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreateextensionHandler(cfg),
	}
}
