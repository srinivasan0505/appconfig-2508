...
func GetdeploymentstrategyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetdeploymentstrategyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_deploymentstrategies_DeploymentStrategyId",
		...
		mcp.WithString("DeploymentStrategyId", mcp.Required(), mcp.Description("The ID of the deployment strategy to get.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetdeploymentstrategyHandler(cfg),
	}
}
