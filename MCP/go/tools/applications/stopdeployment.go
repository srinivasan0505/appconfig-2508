...
func StopdeploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateStopdeploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_applications_ApplicationId_environments_EnvironmentId_deployments_DeploymentNumber",
		mcp.WithDescription("Stops a deployment. This API action works only on deployments that have a status of <code>DEPLOYING</code>. This action moves the deployment to a status of <code>ROLLED_BACK</code>."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    StopdeploymentHandler(cfg),
	}
}
