...
func CreateapplicationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateApplicationrequest
		...
		url := fmt.Sprintf("%s/applications", cfg.BaseURL)
...
func CreateCreateapplicationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_applications",
		...
		mcp.WithString("Description", mcp.Description("Input parameter: A description of the application.")),
		mcp.WithString("Name", mcp.Required(), mcp.Description("Input parameter: A name for the application.")),
		mcp.WithObject("Tags", mcp.Description("Input parameter: Metadata to assign to the application. Tags help organize and categorize your AppConfig resources. Each tag consists of a key and an optional value, both of which you define.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreateapplicationHandler(cfg),
	}
}
