...
func StartdeploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/applications/%s/environments/%s/deployments", cfg.BaseURL, ApplicationId, EnvironmentId)
...
func CreateStartdeploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_applications_ApplicationId_environments_EnvironmentId_deployments",
		mcp.WithDescription("Starts a deployment."),
		...
		mcp.WithString("EnvironmentId", mcp.Required(), mcp.Description("The environment ID.")),
		mcp.WithString("KmsKeyIdentifier", mcp.Description("Input parameter: The KMS key identifier (key ID, key alias, or key ARN). AppConfig uses this ID to encrypt the configuration data using a customer managed key. ")),
		mcp.WithObject("Tags", mcp.Description("Input parameter: Metadata to assign to the deployment. Tags help organize and categorize your AppConfig resources. Each tag consists of a key and an optional value, both of which you define.")),
		mcp.WithString("ConfigurationProfileId", mcp.Required(), mcp.Description("Input parameter: The configuration profile ID.")),
		mcp.WithString("ConfigurationVersion", mcp.Required(), mcp.Description("Input parameter: The configuration version to deploy. If deploying an AppConfig hosted configuration version, you can specify either the version number or version label.")),
		mcp.WithString("DeploymentStrategyId", mcp.Required(), mcp.Description("Input parameter: The deployment strategy ID.")),
		mcp.WithString("Description", mcp.Description("Input parameter: A description of the deployment.")),
	)
