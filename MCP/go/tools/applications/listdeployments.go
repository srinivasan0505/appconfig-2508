...
func ListdeploymentsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/applications/%s/environments/%s/deployments%s", cfg.BaseURL, ApplicationId, EnvironmentId, queryString)
...
func CreateListdeploymentsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_applications_ApplicationId_environments_EnvironmentId_deployments",
		mcp.WithDescription("Lists the deployments for an environment in descending deployment number order."),
		...
		mcp.WithNumber("max_results", mcp.Description("The maximum number of items that may be returned for this call. If there are items that have not yet been returned, the response will include a non-null <code>NextToken</code> that you can provide in a subsequent call to get the next set of results.")),
		mcp.WithString("next_token", mcp.Description("The token returned by a prior call to this operation indicating the next set of results to be returned. If not specified, the operation will return the first set of results.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListdeploymentsHandler(cfg),
	}
}
