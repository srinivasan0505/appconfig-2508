...
func GetdeploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		DeploymentNumberVal, ok := args["DeploymentNumber"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: DeploymentNumber"), nil
		}
		DeploymentNumber, ok := DeploymentNumberVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: DeploymentNumber"), nil
		}
		url := fmt.Sprintf("%s/applications/%s/environments/%s/deployments/%s", cfg.BaseURL, ApplicationId, EnvironmentId, DeploymentNumber)
...
func CreateGetdeploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_applications_ApplicationId_environments_EnvironmentId_deployments_DeploymentNumber",
		mcp.WithDescription("Retrieves information about a configuration deployment."),
		...
		mcp.WithString("ApplicationId", mcp.Required(), mcp.Description("The ID of the application that includes the deployment you want to get. ")),
		mcp.WithString("EnvironmentId", mcp.Required(), mcp.Description("The ID of the environment that includes the deployment you want to get. ")),
		mcp.WithNumber("DeploymentNumber", mcp.Required(), mcp.Description("The sequence number of the deployment.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetdeploymentHandler(cfg),
	}
}
