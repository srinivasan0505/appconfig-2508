...
func GethostedconfigurationversionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGethostedconfigurationversionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_applications_ApplicationId_configurationprofiles_ConfigurationProfileId_hostedconfigurationversions_VersionNumber",
		mcp.WithDescription("Retrieves information about a specific configuration version."),
		...
		mcp.WithNumber("VersionNumber", mcp.Required(), mcp.Description("The version.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GethostedconfigurationversionHandler(cfg),
	}
}
