...
func DeleteenvironmentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/applications/%s/environments/%s", cfg.BaseURL, ApplicationId, EnvironmentId)
...
func CreateDeleteenvironmentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_applications_ApplicationId_environments_EnvironmentId",
		mcp.WithDescription("Deletes an environment. Deleting an environment does not delete a configuration from a host."),
		...
		mcp.WithString("ApplicationId", mcp.Required(), mcp.Description("The application ID that includes the environment that you want to delete.")),
		mcp.WithString("EnvironmentId", mcp.Required(), mcp.Description("The ID of the environment that you want to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteenvironmentHandler(cfg),
	}
}
