...
func UpdateconfigurationprofileHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateConfigurationProfilerequest
		...
		url := fmt.Sprintf("%s/applications/%s/configurationprofiles/%s", cfg.BaseURL, ApplicationId, ConfigurationProfileId)
...
func CreateUpdateconfigurationprofileTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_applications_ApplicationId_configurationprofiles_ConfigurationProfileId",
		mcp.WithDescription("Updates a configuration profile."),
		...
		mcp.WithString("ConfigurationProfileId", mcp.Required(), mcp.Description("The ID of the configuration profile.")),
		mcp.WithString("Description", mcp.Description("Input parameter: A description of the configuration profile.")),
		mcp.WithString("Name", mcp.Description("Input parameter: The name of the configuration profile.")),
		mcp.WithString("RetrievalRoleArn", mcp.Description("Input parameter: The ARN of an IAM role with permission to access the configuration at the specified <code>LocationUri</code>.")),
		mcp.WithArray("Validators", mcp.Description("Input parameter: A list of methods for validating the configuration.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdateconfigurationprofileHandler(cfg),
	}
}
