...
func DeletehostedconfigurationversionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		VersionNumberVal, ok := args["VersionNumber"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: VersionNumber"), nil
		}
		VersionNumber, ok := VersionNumberVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: VersionNumber"), nil
		}
		url := fmt.Sprintf("%s/applications/%s/configurationprofiles/%s/hostedconfigurationversions/%s", cfg.BaseURL, ApplicationId, ConfigurationProfileId, VersionNumber)
...
func CreateDeletehostedconfigurationversionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_applications_ApplicationId_configurationprofiles_ConfigurationProfileId_hostedconfigurationversions_VersionNumber",
		mcp.WithDescription("Deletes a version of a configuration from the AppConfig hosted configuration store."),
		...
		mcp.WithNumber("VersionNumber", mcp.Required(), mcp.Description("The versions number to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletehostedconfigurationversionHandler(cfg),
	}
}
