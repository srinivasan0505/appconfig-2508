...
func GetenvironmentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetenvironmentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_applications_ApplicationId_environments_EnvironmentId",
		...
		mcp.WithString("ApplicationId", mcp.Required(), mcp.Description("The ID of the application that includes the environment you want to get.")),
		mcp.WithString("EnvironmentId", mcp.Required(), mcp.Description("The ID of the environment that you want to get.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetenvironmentHandler(cfg),
	}
}
