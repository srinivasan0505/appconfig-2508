...
func DeleteapplicationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteapplicationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_applications_ApplicationId",
		mcp.WithDescription("Deletes an application. Deleting an application does not delete a configuration from a host."),
		...
		mcp.WithString("ApplicationId", mcp.Required(), mcp.Description("The ID of the application to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteapplicationHandler(cfg),
	}
}
