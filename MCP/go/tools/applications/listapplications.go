...
func ListapplicationsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/applications%s", cfg.BaseURL, queryString)
		...
		var result models.Applications
...
func CreateListapplicationsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_applications",
		mcp.WithDescription("Lists all applications in your Amazon Web Services account."),
		...
		mcp.WithString("next_token", mcp.Description("A token to start the list. Next token is a pagination token generated by AppConfig to describe what page the previous List call ended on. For the first List request, the nextToken should not be set. On subsequent calls, the nextToken parameter should be set to the previous responses nextToken value. Use this token to get the next set of results. ")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListapplicationsHandler(cfg),
	}
}
