...
func DeleteextensionassociationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeleteextensionassociationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_extensionassociations_ExtensionAssociationId",
		mcp.WithDescription("Deletes an extension association. This action doesn't delete extensions defined in the association."),
		...
		mcp.WithString("ExtensionAssociationId", mcp.Required(), mcp.Description("The ID of the extension association to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteextensionassociationHandler(cfg),
	}
}
