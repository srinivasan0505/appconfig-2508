...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_tags_ResourceArn#tagKeys",
		mcp.WithDescription("Deletes a tag key and value from an AppConfig resource."),
		...
		mcp.WithString("ResourceArn", mcp.Required(), mcp.Description("The ARN of the resource for which to remove tags.")),
		mcp.WithArray("tagKeys", mcp.Required(), mcp.Description("The tag keys to delete.")),
	)
