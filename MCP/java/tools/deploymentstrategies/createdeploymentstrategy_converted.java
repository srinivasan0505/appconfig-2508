...
class Post_DeploymentstrategiesMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_DeploymentstrategiesHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("ReplicateTo")) {
            queryParams.add("ReplicateTo=" + args.get("ReplicateTo"));
        }
        ...
        if (args.containsKey("Tags")) {
            queryParams.add("Tags=" + args.get("Tags"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_deploymentstrategies" + queryString;
    ...
    public static MCPServer.Tool createPost_DeploymentstrategiesTool(MCPServer.APIConfig config) {
        ...
        GrowthFactorProperty.put("required", true);
        ...
        NameProperty.put("description", "Input parameter: A name for the deployment strategy.");
        properties.put("Name", NameProperty);
        Map<String, Object> ReplicateToProperty = new HashMap<>();
        ReplicateToProperty.put("type", "string");
        ReplicateToProperty.put("required", false);
        ReplicateToProperty.put("description", "Input parameter: Save the deployment strategy to a Systems Manager (SSM) document.");
        properties.put("ReplicateTo", ReplicateToProperty);
        Map<String, Object> DeploymentDurationInMinutesProperty = new HashMap<>();
        DeploymentDurationInMinutesProperty.put("type", "string");
        DeploymentDurationInMinutesProperty.put("required", true);
        ...
        TagsProperty.put("description", "Input parameter: Metadata to assign to the deployment strategy. Tags help organize and categorize your AppConfig resources. Each tag consists of a key and an optional value, both of which you define.");
        properties.put("Tags", TagsProperty);
        parameters.put("properties", properties);
        
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "post_deploymentstrategies",
            "Creates a deployment strategy that defines important criteria for rolling out your configuration to the designated targets. A deployment strategy includes the overall duration required, a percentage of targets to receive the deployment during each interval, an algorithm that defines how percentage grows, and bake time.",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPost_DeploymentstrategiesHandler(config));
    }
