/**
 * MCP Server function for Lists all applications in your Amazon Web Services account.
 */
...
class Get_ApplicationsMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getGet_ApplicationsHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("max_results")) {
            queryParams.add("max_results=" + args.get("max_results"));
                ...
                String url = config.getBaseUrl() + "/api/v2/get_applications" + queryString;
    ...
    public static MCPServer.Tool createGet_ApplicationsTool(MCPServer.APIConfig config) {
        ...
        next_tokenProperty.put("description", "A token to start the list. Next token is a pagination token generated by AppConfig to describe what page the previous List call ended on. For the first List request, the nextToken should not be set. On subsequent calls, the nextToken parameter should be set to the previous responses nextToken value. Use this token to get the next set of results.");
        ...
        return new MCPServer.Tool(definition, getGet_ApplicationsHandler(config));
    }
